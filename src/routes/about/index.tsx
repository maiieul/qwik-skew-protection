...
export const aboutGreeter = $(({ name2 }: { name2: string }) => {
  if (!name2) {
    throw new Error("Please provide a name2");
  }

  return `Client says Hi ${name2}`;
});
  ...
  return (
    <>
      ...
      <button
        type="submit"
        onClick$={async () => {
          const message = await aboutGreeter({ name2: "Maieul" });
          alert(message);
        }}
      >
